using System.Collections.Generic;
using System.Threading.Tasks;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using RedPet.API.Extensions;
using RedPet.Common.Models.Pet;
using RedPet.Core;

namespace RedPetAPI.Controllers
{
    [Route("api/[controller]")]
    [ApiVersion("1")]
    [Authorize]
    public class PetSizesController : Controller
    {
        private readonly IPetSizeService petSizeService;
        
        public PetSizesController(IPetSizeService petSizeService)
        {
            this.petSizeService = petSizeService;            
        }
        
        [HttpGet]
        [AllowAnonymous]
        public async Task<ActionResult<List<PetSizeModel>>> GetAsync()
        {
            var result = await petSizeService.GetAsync();
            return result.ConvertToActionResult(System.Net.HttpStatusCode.OK);
        }
    }
}
